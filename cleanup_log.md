...
## 2026-07-24 — Succès × cheat unlock (T-21) + déplacement LevelData → Resources

- **`Assets/Scriptable Objects/Levels/` → `Assets/Resources/Levels/`** — *(déplacé, git mv)* 8 assets `LevelData` (Level1-6 + Boss1/2) + .meta. GUIDs inchangés → références scène/planches intactes. Raison : rendre le catalogue de niveaux chargeable partout (`Resources.LoadAll`), indépendamment des managers de scène.
- **`Assets/Scripts/Game/LevelCatalog.cs`** — *(créé)* statique, `GetAll()` = LoadAll("Levels") trié par levelID, cache. Source unique pour succès + cheat.
- **`AchievementStore.cs`** — *(modifié)* conditions rebasées sur `LevelCatalog.GetAll()` ; **bug silencieux T-14 réparé** : l'exigence `LevelSelectManager.Instance != null` (panel 2D désactivé → Instance jamais setté) rendait les succès de niveaux indébloquables, même en jeu légitime. `using Menus;` retiré.
- **`Player.cs`** — *(modifié)* `UnlockAllLevels()` marque tous les niveaux du catalogue comme terminés (repli 0..7) + `AchievementStore.EvaluateAll()` immédiat.
- **`CheatConsole.cs`** — *(1 ligne)* feedback « débloqués + terminés (succès inclus) ».
- **⚠️ PIÈGE (anti-régression)** : compiler PENDANT le Play tue les singletons statiques (`Player.Instance == null`, Awake non rejoué après domain reload) → toujours stop/play avant un test MCP. Symptôme vu : « Already in play mode » + NRE.
- **Testé en Play (MCP)** : catalogue 8 niveaux, unlock → 4 succès True. La sauvegarde de test a tout débloqué → Wipe save au menu pour repartir à zéro.

### À FAIRE / à surveiller
- `LevelSelectManager.GetOrderedLevels()` n'a plus d'appelant connu (conservé, API publique UI) — suppression éventuelle = demander d'abord.
- Si un 9e niveau est ajouté : poser l'asset dans `Assets/Resources/Levels/`, rien d'autre à câbler (catalogue auto).
